...
class OperationCancelled implements Exception {
  ...
  OperationCancelled([this.message = 'Operation cancelled']);
  @override
...
class MusicPlayer {
  ...
  final bool confExistedBefore = await confFile.exists();
  if (!overwrite && confExistedBefore) {
  ...
  final bool oauthDirExistedBefore = await oauthDir.exists();
  await oauthDir.create(recursive: true);

  final File credsFile = File(p.join(cachePath, 'oauth', 'credentials.json'));
  final bool credsExistedBefore = await credsFile.exists();
  ...
  // If cancelled before writing, bail early
  ...
  // If creds already exist, we’re done (no auth needed)
  if (credsExistedBefore) return;
  ...
  Timer? pollTimer;
  pollTimer = Timer.periodic(const Duration(milliseconds: 300), (_) async {
    // Cancellation check
    if (cancelToken?.isCancelled == true) {
      if (!completer.isCompleted) {
        completer.completeError(OperationCancelled());
      }
      return;
    }
  ...
  try {
    ...
    // Cleanup ONLY what we created (best-effort)
    try {
      if (!confExistedBefore && await confFile.exists()) {
        await confFile.delete();
      }
      // Delete cache dir only if we created it (oauthDir didn’t exist before)
      if (!oauthDirExistedBefore) {
        final Directory cacheDir = Directory(cachePath);
        if (await cacheDir.exists()) await cacheDir.delete(recursive: true);
      } else {
        if (!credsExistedBefore && await credsFile.exists()) {
          await credsFile.delete();
        }
      }
    } catch (_) {}
